...
(Note: I'm going to assume you're familiar with the beta and binomial distributions, since what we are about to do is an extension of those ideas. If you're not familiar, you'll probably still get the gist, but might need to do some background research to understand the technical aspects of what we're doing. Will Kurt's blog post [*Working with Probability Distributions*](https://www.countbayesie.com/blog/2015/3/17/interrogating-probability-distributions) and David Robinson's blog post [*Understanding the beta distribution (using baseball statistics)*](http://varianceexplained.org/statistics/beta_distribution_and_baseball/) are great primers.)

### Distribution of star ratings

- Picking an overall rating.
- Need to estimate parameters for what goes into calculating the overall rating: number of ratings at each star level.
- Binomial distribution describes situations with two outcomes -- in our case, might be recommend book or don't recommend book.
- Two parameters for binomial distribution: probability of success and number of trials. Beta distribution describes the range of values for the probability parameter
- Need to extend the binomial distribution to include multiple outcomes -- in our case, instead of recommend/don't recommend, we have five outcomes - 1-star rating, 2-star rating, etc.
- Just as the beta distribution describes the range of values for probability of success in the binomial distribution, the Dirichlet distribution describes the range of values for the probability of each outcome being chosen for the multinomial distribution.
- We can get an intuitive sense of this by looking at our ratings, but instead of looking at the *number* of ratings at each star level, we'll looking at *percent* of ratings for that book that are each star level.
...
- Note that we're only looking at books with more than 500 reviews so that we're only looking at books with fairly "settled", solid ratings
- It's clear that the percentage of a book's ratings at each star level follows a pattern: 2-star ratings are likely to comprise 0-15% of ratings and 4-star ratings are likely comprise 20-50% of ratings
- We can extend this idea to probabilities -- if a book got a new rating, we could look at these distributions and give a rough probability for the rating being at each star level:
- 5: 45%
- 4: 30%
- 3: 20%
- 2: 4%
...
### Maximum likelihood estimation with the dirichlet distribution
