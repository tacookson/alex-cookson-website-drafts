...
1. **Normalizing** and **re-scaling** ratings so that they are more interpretable (and look more like that nice, symmetrical normal curve pictured above) (*coming soon*)  
\  
...
First, we'll load our packages and import the data. We're going to use a dataset set of about 10,000 children's books that have been rated from 1-5 stars. In terms of packages, in addition to the `tidyverse`, we'll use:
...
books %>%
  sample_n(8) %>%
  # knitr and kableExtra for formatted tables
...
So a book with zero ratings, using our empirical Bayes estimate, will have about **`r comma(par_total, accuracy = 1)` "starter ratings"** and an **overall rating of `r comma(par_mean, accuracy = 0.01)`**. Again, this will change as a books gets real ratings, but it will be less prone to, for example, a few good ratings resulting in an overall rating of 5 stars.
